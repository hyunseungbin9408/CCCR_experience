 ...
 ## ovirt관리모드에서 데이터 센터생성
  ...
  <img src="https://github.com/hyunseungbin9408/CCCR_experience/blob/master/png/new_cluster2.png" alt="drawing" width="450"/>
   ...
   ```
    ...
    메모리 페이지 공유 -> 메모리 오버커밋
    오버헤드 또는 페이지 폴트 발생원인
    KSM을 사용하면 하이퍼바이저가 동일한 메모리 페이지를 통합하여 동시 가상 시스템 수를 늘릴 수 있다.
...
***
 ...
 + 작업부하 분산(Load-Balancing) 클러스터는 시스템의 전반적인 성능을 향상시키기 위해 계산 부하량을 여러 노드에서 분담하여 병렬 처리하도록 구성 하는 방식이다.
   + 웹서버에 작업부하 분산 클러스터를 적용하면 각기 다른 응답들을 서버마다 각각 처리할 수 있어서 웹서버의 응답시간을 단축시킬 수 있다.
