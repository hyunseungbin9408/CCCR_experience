...
ZFS에서는 데이터의 중복을 위해 기존 파일시스템의 RAID 개념을 가지고 있는 RAID-Z가 있다. 전통적인 RAID와 비슷하지만 RAID-Z 의 차이점은 가변적인 스트라이프를 지원함으로서 전통적인 RAID-5의 Write-hole의 오류를 제거했다. 모든 RAID-Z는 스트라이프방식으로 쓰기한다.

세 개 이상의 디스크에 데이터와 패리티 비트를 저장하는 RAID-5는 ZFS의 RAID-Z와 대응이 되며, 네 개 이상의 디스크에 데이터와 더블 패리티 비트를 저장하는 RAID-6는 RAID-Z2와 대응된다.

### 7. 스냅샷과 복제

스냅샷은 파일시스템이나 볼륨의 I/O 가 발생하고 있는 동안에도 파일시스템이나 볼륨의 현재 내용의 읽기 전용 복사본을 생성 할 수 있다. 또한 읽기 전용 복사본인 스냅샷에 쓰기 기능을 추가한 것이 복제이다.

### 8. 롤백과 백업

롤백을 사용하여 현재 활성화된 ZFS 파일 시스템을 이전에 생성해 놓은 스냅샷으로 되돌릴 수 있다. 당연히 스냅샷을 생성한 이후의 데이터는 손실된다. 이런 스냅샷을 이용하여 현재 활성화된 파일시스템을 오프라인이 아닌 온라인 상태에서 백업 할 수 있다.

### 9.압축

압축은 모든 파일을 크기를 줄여 파일 전송이나, 공간을 효율적으로 사용하도록 도와준다. 기본적으로 압축 기능을 사용하지 않지만, 압축이 필요하다면 활성화 시킬 수 있다.
...
+ `[type]`: ZFS 스토리지 풀의 유형을 지정한다. 지정하지 않으면 기본 값으로 스트라이프 형태의 기본 스토리지 풀이 지정된다.
