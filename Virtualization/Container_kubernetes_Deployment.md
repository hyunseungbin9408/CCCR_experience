...
## 디플로이먼트
### 디플로이먼트 정의
+ 지금까지 파드를 제어하는 컨트롤러 `ReplicaSet`보다 상위 수준인 컨트롤러

+ 애플리케이션을 배포하고 선언적으로 업데이트를 수행하는 리소스

+ `ReplicaSet`을 제어하고 그 레플리카셋이 파드를 제어하는 
...
+ `kubectl apply -f yaml파일명`으로도 업데이트를 할 수 있는데 `apply`로 업데이트를 하는경우에는 그전 버전에 정보들이 `history`에 들어가게된다.
